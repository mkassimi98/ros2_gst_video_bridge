...
{
    return gst_format == "rggb" || gst_format == "bggr" || gst_format == "gbrg" || gst_format == "grbg";
}
        ...
        uint64_t           timestamp_ns)
{
    ...
    if (!running_.load() || appsrc_ == nullptr || pipeline_element_ == nullptr || data == nullptr || size == 0) {
        return false;
    }
...
uint64_t StreamEngine::normalizeBufferPts(uint64_t source_timestamp_ns)
{
    ...
    uint64_t pts_ns = 0;
    if (source_timestamp_ns > 0 && base_source_timestamp_ns_ > 0 && source_timestamp_ns >= base_source_timestamp_ns_) {
        pts_ns = source_timestamp_ns - base_source_timestamp_ns_;
    } else {
        pts_ns = static_cast<uint64_t>(
                std::chrono::duration_cast<std::chrono::nanoseconds>(now - first_frame_steady_time_).count());
    }
...
bool StreamEngine::processBusMessages(GstClockTime timeout)
{
    ...
    GstMessage* message
            = timeout == 0 ? gst_bus_pop_filtered(bus_, filter) : gst_bus_timed_pop_filtered(bus_, timeout, filter);
