...
std::vector<std::string> CapabilityProbe::detectPlugins() const {
  ...
  const auto codec_plugins = filterAvailableElements({{"av1enc", "AV1 encoder"},
...
std::vector<std::string> CapabilityProbe::detectEncoderImplementations() const {
  if (!hasGstInspect()) {
    return {"av1 -> av1enc [sw]", "h265 -> x265enc [sw]", "h264 -> x264enc [sw]",
            "mjpeg -> jpegenc [sw]"};
  }

  return filterAvailableCodecElements({{"av1enc", "av1", "sw"},
                                       {"rav1enc", "av1", "sw"},
                                       {"svtav1enc", "av1", "sw"},
