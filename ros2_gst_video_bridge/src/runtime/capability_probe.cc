...
filterAvailableElements(const std::vector<std::pair<std::string, std::string>>& candidates) {
  std::vector<std::string> available;
  for (const auto& candidate : candidates) {
...
std::vector<std::string> CapabilityProbe::detectPlugins() const {
  ...
  const auto core_plugins = filterAvailableElements({{"appsrc", "appsrc"},
                                                     {"videoconvert", "videoconvert"},
                                                     {"videoscale", "videoscale"},
                                                     {"queue", "queue"}});
  plugins.insert(plugins.end(), core_plugins.begin(), core_plugins.end());

  const auto transport_plugins = filterAvailableElements({{"srtsink", "SRT sink"},
  ...
  const auto codec_plugins = filterAvailableElements(
      {{"x264enc", "H264 encoder"}, {"x265enc", "H265 encoder"}, {"jpegenc", "MJPEG encoder"}});
...
std::vector<std::string> CapabilityProbe::detectHostProfile() const {
  ...
  profile.push_back("runtime_arch=" +
                    (runtime_arch.empty() ? std::string("unknown") : runtime_arch));
...
std::vector<std::string> CapabilityProbe::detectEncoderImplementations() const {
  if (!hasGstInspect()) {
    return {"h264 -> x264enc [sw]", "h265 -> x265enc [sw]", "mjpeg -> jpegenc [sw]"};
  }

  return filterAvailableCodecElements({{"x264enc", "h264", "sw"},
                                       {"openh264enc", "h264", "sw"},
                                       {"x265enc", "h265", "sw"},
                                       {"jpegenc", "mjpeg", "sw"},
