...
MetricsPublisher::MetricsPublisher(rclcpp::Node& node) : node_(node)
{
    ...
    status_pub_
            = node_.create_publisher<ros2_gst_video_bridge_msgs::msg::RuntimeStatus>("~/runtime_status", status_qos);
    event_pub_ = node_.create_publisher<ros2_gst_video_bridge_msgs::msg::RuntimeEvent>("~/runtime_events", event_qos);

    last_publish_time_ns_ = static_cast<uint64_t>(node_.now().nanoseconds());
    publish_timer_        = node_.create_wall_timer(std::chrono::seconds(1), [this]() { publishSnapshot(); });
}
...
void MetricsPublisher::setCodecSelectionFlags(bool auto_codec_requested, bool hw_selected, bool sw_fallback_applied)
{
...
void MetricsPublisher::recordPushLatency(double latency_ms)
{
    ...
    const double     next     = previous <= 0.0 ? latency_ms : ((1.0 - alpha) * previous + alpha * latency_ms);
    ...
    while (latency_ms > observed_max
           && !push_latency_max_ms_.compare_exchange_weak(observed_max, latency_ms, std::memory_order_relaxed)) {}
}
...
void MetricsPublisher::publishEvent(const std::string& severity, const std::string& code, const std::string& message)
{
...
ros2_gst_video_bridge_msgs::msg::RuntimeStatus MetricsPublisher::buildStatusSnapshot(uint64_t now_ns, double elapsed_s)
{
    ...
    status.fps_out                 = static_cast<float>(static_cast<double>(out_count - last_frames_out_) / elapsed_s);
    ...
    status.latency_estimate_ms        = static_cast<float>(latency_estimate_ms_.load(std::memory_order_relaxed));
    status.push_latency_estimate_ms   = static_cast<float>(push_latency_estimate_ms_.load(std::memory_order_relaxed));
    status.push_latency_max_ms        = static_cast<float>(push_latency_max_ms_.load(std::memory_order_relaxed));
...
std::string MetricsPublisher::buildLegacyPayload(
        const ros2_gst_video_bridge_msgs::msg::RuntimeStatus& status,
        uint64_t                                              drops_since_last)
{
    std::ostringstream payload;
    payload << "state=" << status.state << " fps_in=" << status.fps_in << " fps_out=" << status.fps_out
            << " dropped_total=" << status.dropped_total << " dropped_since_last=" << drops_since_last
            ...
            << " reconnect_count=" << status.reconnect_count << " latency_estimate_ms=" << status.latency_estimate_ms
