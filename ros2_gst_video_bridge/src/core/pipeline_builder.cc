...
std::string buildEncoderChain(
        const GstBridgeConfig& config,
        ...
        const std::string&     selected_encoder)
{
    const std::string encoder
            = selected_encoder.empty()
                    ...
                    : selected_encoder;
    const std::string input_preprocess = config.source.detected_bayer_format.empty() ? "" : "bayer2rgb ! ";
...
void appendPayloadOrMux(std::ostringstream& pipeline, const std::string& transport, const std::string& codec)
{
