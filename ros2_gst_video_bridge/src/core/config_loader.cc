...
std::string forceSrtListenerMode(const std::string& sink_uri)
{
    ...
    normalized.replace(
            value_begin, value_end == std::string::npos ? std::string::npos : value_end - value_begin, "listener");
    return normalized;
}
...
GstBridgeConfig ConfigLoader::loadFromNode(rclcpp::Node& node)
{
    ...
    config.profile.machine = node.declare_parameter<std::string>("profile.machine", config.profile.machine);
    config.profile.stream  = node.declare_parameter<std::string>("profile.stream", config.profile.stream);

    applyMachineProfileDefaults(config);
    applyStreamProfileDefaults(config);

    config.source.input_topic = node.declare_parameter<std::string>("input_topic", config.source.input_topic);

    config.transport.kind     = node.declare_parameter<std::string>("gst.transport", config.transport.kind);
    config.transport.kind     = node.declare_parameter<std::string>("transport.kind", config.transport.kind);
    config.transport.sink_uri = node.declare_parameter<std::string>("gst.sink_uri", config.transport.sink_uri);
    config.transport.sink_uri = node.declare_parameter<std::string>("transport.sink_uri", config.transport.sink_uri);
    if (config.transport.kind == "srt") {
        config.transport.sink_uri = forceSrtListenerMode(config.transport.sink_uri);
    }
    config.transport.latency_ms = node.declare_parameter<int>("gst.latency_ms", config.transport.latency_ms);
    config.transport.latency_ms = node.declare_parameter<int>("transport.latency_ms", config.transport.latency_ms);
    ...
    config.codec.rate_control = node.declare_parameter<std::string>("codec.rate_control", config.codec.rate_control);
    config.codec.bitrate_kbps = node.declare_parameter<int>("gst.bitrate_kbps", config.codec.bitrate_kbps);
    config.codec.bitrate_kbps = node.declare_parameter<int>("codec.bitrate_kbps", config.codec.bitrate_kbps);
    ...
    config.runtime.use_wall_clock_timestamps
            = node.declare_parameter<bool>("use_wall_clock_timestamps", config.runtime.use_wall_clock_timestamps);
    ...
    config.runtime.print_effective_config
            = node.declare_parameter<bool>("runtime.print_effective_config", config.runtime.print_effective_config);
    config.runtime.backpressure_reconnect_after_ms = node.declare_parameter<int>(
            "runtime.backpressure.reconnect_after_ms", config.runtime.backpressure_reconnect_after_ms);
    config.runtime.backpressure_max_consecutive_drops = node.declare_parameter<int>(
            "runtime.backpressure.max_consecutive_drops", config.runtime.backpressure_max_consecutive_drops);
...
std::vector<std::string> ConfigLoader::validate(const GstBridgeConfig& config)
{
    ...
    if (!inSet(config.transport.kind, valid_transports)) {
        errors.emplace_back(
                "transport.kind must be one of [srt, rtsp, udp, file]. Current value: '" + config.transport.kind
                + "'.");
    }

    if (config.gst.pipeline_override.empty() && config.transport.sink_uri.empty()) {
        errors.emplace_back("transport.sink_uri cannot be empty unless gst.pipeline_override is provided.");
    }

    if (!config.gst.pipeline_override.empty()) {
        // Custom pipeline override takes full responsibility for compatibility checks.
    } else if (config.transport.kind == "srt" && !detail::startsWith(config.transport.sink_uri, "srt://")) {
        errors.emplace_back("transport.sink_uri must start with 'srt://' when transport.kind=srt.");
    } else if (config.transport.kind == "rtsp" && !detail::startsWith(config.transport.sink_uri, "rtsp://")) {
        errors.emplace_back("transport.sink_uri must start with 'rtsp://' when transport.kind=rtsp.");
    } else if (config.transport.kind == "udp" && !detail::startsWith(config.transport.sink_uri, "udp://")) {
        errors.emplace_back("transport.sink_uri must start with 'udp://' when transport.kind=udp.");
    } else if (config.transport.kind == "file" && detail::startsWith(config.transport.sink_uri, "udp://")) {
        errors.emplace_back("transport.sink_uri for transport.kind=file must be a file path, not UDP URI.");
    }
    ...
    if (!inSet(config.codec.name, valid_codecs)) {
        errors.emplace_back(
                "codec.name must be one of [auto, av1, h265, h264, mjpeg]. Current value: '" + config.codec.name
                + "'.");
    }
    ...
    if (config.runtime.backpressure_reconnect_after_ms <= 0) {
        errors.emplace_back("runtime.backpressure.reconnect_after_ms must be > 0.");
    }

    if (config.runtime.backpressure_max_consecutive_drops <= 0) {
        errors.emplace_back("runtime.backpressure.max_consecutive_drops must be > 0.");
    }
...
std::string ConfigLoader::toDebugString(const GstBridgeConfig& config)
{
    ...
    out << "runtime.backpressure.reconnect_after_ms=" << config.runtime.backpressure_reconnect_after_ms << '\n';
    out << "runtime.backpressure.max_consecutive_drops=" << config.runtime.backpressure_max_consecutive_drops << '\n';
