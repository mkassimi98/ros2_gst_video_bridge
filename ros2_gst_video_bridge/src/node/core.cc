...
std::string detectMachineProfile(const std::string& configured_machine, const std::vector<std::string>& host_profile)
{
    ...
    if (runtime_arch == "x86_64" || runtime_arch == "amd64" || runtime_arch == "x86" || runtime_arch == "i386") {
        return "x86";
    }
...
std::string selectAutoCodec(
        const std::string&              machine,
        const std::vector<std::string>& implementations,
        std::string&                    reason)
{
    const std::vector<std::pair<std::string, int>> codec_priority{{"av1", 4}, {"h265", 3}, {"h264", 2}, {"mjpeg", 1}};
...
GstVideoBridgeNode::GstVideoBridgeNode(const rclcpp::NodeOptions& options) : Node("gst_video_bridge", options)
{
    ...
    adaptation_profile_     = this->declare_parameter<std::string>("runtime.adaptation.profile", "balanced");
    ...
    const auto transport_preflight = detail::resolveTransportForAvailableSinks(config_.transport.kind, available_sinks);
    ...
    if (config_.codec.name == "auto") {
        ...
        RCLCPP_INFO(
                ...
                config_.codec.encoder.c_str(),
                selection_reason.c_str(),
                machine_for_auto.c_str());
    } else {
        hw_encoder_selected_ = false;

        const auto encoder_preflight
                = detail::resolveEncoderOverride(config_.codec.name, config_.codec.encoder, encoder_implementations_);
