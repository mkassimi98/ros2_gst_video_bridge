...
bool GstVideoBridgeNode::handleRuntimeMode()
{
    ...
    RCLCPP_ERROR(this->get_logger(), "Unsupported runtime.mode value: %s", config_.runtime.mode.c_str());
    return true;
}
...
void GstVideoBridgeNode::printGstCapabilities() const
{
    if (!capability_probe_->hasGstInspect()) {
        RCLCPP_WARN(
                this->get_logger(), "gst-inspect-1.0 was not found on PATH. Reporting static fallback capabilities.");
    }
    ...
    RCLCPP_INFO(this->get_logger(), "Detected encoder implementations (CPU/HW): %zu", encoder_impls.size());
