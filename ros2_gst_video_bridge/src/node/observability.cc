...
void GstVideoBridgeNode::setRuntimeState(RuntimeState new_state, const std::string& reason)
{
    ...
    if (new_state == RuntimeState::Degraded || new_state == RuntimeState::Failed) {
        RCLCPP_WARN(
                this->get_logger(), "runtime.state=%s reason=%s", runtimeStateToString(runtime_state_), reason.c_str());
    } else {
        RCLCPP_INFO(
                this->get_logger(), "runtime.state=%s reason=%s", runtimeStateToString(runtime_state_), reason.c_str());
    }
...
void GstVideoBridgeNode::publishRuntimeEvent(
        ...
        std::shared_ptr<ros2_gst_video_bridge_msgs::srv::SetStreamingProfile::Response>      response)
{
    ...
    publishRuntimeEvent("info", "OPERATOR_PROFILE_UPDATE", "runtime profile updated via service request");
...
const char* GstVideoBridgeNode::runtimeStateToString(RuntimeState state)
{
    switch (state) {
        ...
        case RuntimeState::Reconnecting: return "reconnecting";
...
void GstVideoBridgeNode::logConfiguration() const
{
    ...
    RCLCPP_INFO(
            this->get_logger(),
            "transport.reconnect.enabled: %s",
            config_.transport.reconnect_enabled ? "true" : "false");
    RCLCPP_INFO(this->get_logger(), "transport.reconnect.interval_ms: %d", config_.transport.reconnect_interval_ms);
    RCLCPP_INFO(this->get_logger(), "transport.reconnect.max_attempts: %d", config_.transport.reconnect_max_attempts);
    ...
    RCLCPP_INFO(this->get_logger(), "gst.pipeline_override: %s", config_.gst.pipeline_override.c_str());

    RCLCPP_INFO(this->get_logger(), "max_fps: %.2f", config_.runtime.max_fps);
    RCLCPP_INFO(
            this->get_logger(),
            "use_wall_clock_timestamps: %s",
            config_.runtime.use_wall_clock_timestamps ? "true" : "false");
    RCLCPP_INFO(
            this->get_logger(),
            "runtime.backpressure.reconnect_after_ms: %d",
            config_.runtime.backpressure_reconnect_after_ms);
    RCLCPP_INFO(
            this->get_logger(),
            "runtime.backpressure.max_consecutive_drops: %d",
            config_.runtime.backpressure_max_consecutive_drops);
...
void GstVideoBridgeNode::logRuntimeCapabilities() const
{
    ...
    RCLCPP_INFO(this->get_logger(), "detected_encoder_implementations: %s", encoder_impl_stream.str().c_str());
