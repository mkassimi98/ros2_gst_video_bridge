...
void GstVideoBridgeNode::evaluateAdaptationPolicy()
{
    ...
    } else if (runtime_state_ == RuntimeState::Streaming && reconnect_count_ == 0 && adaptation_level_ > 0) {
...
void GstVideoBridgeNode::applyAdaptationLevel(uint8_t level, const std::string& reason)
{
    ...
    const int    next_gop     = std::max(10, static_cast<int>(base_gop_ * (adaptation_level_ == 0 ? 1.0 : 0.8)));
...
void GstVideoBridgeNode::scheduleReconnect(const std::string& reason, bool allow_software_fallback)
{
...
bool GstVideoBridgeNode::tryReconnect()
{
    ...
    last_throttle_steady_time_      = std::chrono::steady_clock::time_point{};
