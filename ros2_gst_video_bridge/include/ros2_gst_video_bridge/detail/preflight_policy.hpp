...
inline bool hasTransport(const std::vector<std::string>& available_sinks,
...
inline TransportPreflightResult resolveTransportForAvailableSinks(
    const std::string& requested_transport, const std::vector<std::string>& available_sinks) {
  ...
  for (const auto* candidate : kFallbackOrder) {
    if (hasTransport(available_sinks, candidate)) {
      return {candidate, true,
              "transport '" + requested_transport + "' unavailable, using '" + candidate +
                  "' detected on host"};
    }
  }
...
inline bool hasEncoderImplementationForCodec(const std::vector<std::string>& implementations,
...
inline EncoderPreflightResult
resolveEncoderOverride(const std::string& codec, const std::string& requested_encoder,
  ...
  if (!sw_encoder.empty()) {
    return {sw_encoder, true,
            "encoder '" + requested_encoder + "' unavailable, using software fallback '" +
                sw_encoder + "'"};
  }
